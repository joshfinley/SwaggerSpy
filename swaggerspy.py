...
def get_urls(search_term):
    ...
    response = session.get(
        base_url.format(search_term, 0), headers={"accept": "application/json"}
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: python3 swaggerspy.py test.com",
    )

    parser.add_argument("searchterm", help="Search term (more accurate with domains)")
    parser.add_argument(
        "-t",
        "--threads",
        type=int,
    ...
    parser.add_argument(
        "-o", "--outfile", type=str, default="", help="Log results to JSON"
    )
    ...
    result_data = {
        "search_term": args.searchterm,
        "search_results:": result_urls,
        "matches": [],
    }

    if result_urls:
        print("[*] Found {} urls".format(len(result_urls)))
    else:
        print("[!] No results")
        return 2
    ...
    with ThreadPoolExecutor(max_workers=args.threads) as executor:
        future_to_url = {
            executor.submit(process_url, url, regex_patterns): url
    ...
    if args.outfile:
        with open(args.outfile, "w") as outfile:
            ...
            outfile.write(json.dumps(result_data, indent=2))
